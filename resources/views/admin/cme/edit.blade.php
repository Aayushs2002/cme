...
@section('body')
    ...
    <div class="row  w-[80%]">
        <form method="post" action="{{ route('admin.cme.update',$program->id) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="mt-5">
            ...
            <div class="mt-5">
                ...
                <input type="date" name="start_date" id="date"
                    class="w-full border py-2 mt-1 outline-none px-3 rounded-md" value="{{old('start_date',$program->start_date)}}">
                ...
                <div class="mt-5">
                    ...
                    <select name="organization_id" class="w-full border py-2 mt-1 text-black outline-none px-3 rounded-md">
                        ...
                        @foreach ($organizations as $organization)
                            <option class="text-black" value="{{ $organization->id }}" {{ old('organization_id', $program->organization_id) == $organization->id? 'selected' : '' }}>{{ $organization->name }}</option>
                        @endforeach
                    </select>
            @endif
    </div>
    ...
    <div class="mt-5">
        ...
        <div class="flex items-center mb-4">
            <input id="default-radio-1" type="radio" value="1" name="status"
                ...
                {{ old('status', $program->status) == '1' ? 'checked' : '' }}>
            <label for="default-radio-1" class="ms-2 text-sm font-medium text-gray-900">On Going</label>
        </div>
        <div class="flex items-center">
            <input id="default-radio-2" type="radio" value="0" name="status"
                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500"
                {{ old('status', $program->status) == '0' ? 'checked' : '' }}>
            <label for="default-radio-2" class="ms-2 text-sm font-medium text-gray-900">Upcoming</label>
        </div>
    </div>
